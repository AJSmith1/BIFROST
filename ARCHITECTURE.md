...
- [7] Documentation. A Documenter.jl site builds from `docs/make.jl` over `docs/src/`
  (theory pages plus per-module `@autodocs` API pages); `docs/references/` holds research
  source material and is excluded from the build. `docs/undocumented.md` (regenerated by
  `docs/undocumented.jl`) tracks exported symbols still missing docstrings.
